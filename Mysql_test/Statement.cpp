...
Statement::Statement(MysqlConnect *db, const std::string &sqlStatement) : _db(db) {
...
void Statement::ExecuteQuery(Statement * stmt)
{
    if (! stmt->_db->check_connect()) {
        throw DatabaseException("Error in Statement::Execute", 0, "----", "Database is not connected");
    }

    stmt->_numberAffectedRows = 0;
    stmt->_eof = true;
    stmt->_currentColumn = 0;

    if (stmt->ParameterCount() != stmt->_params.size()) {
        throw DatabaseException("Error in Statement::Execute", 0, "----", "Have not yet assigned all parameters");
    }

    if (mysql_stmt_bind_param(stmt->_stmt, stmt->_bind) != 0) {
        throw DatabaseException(stmt->_stmt, "Error in Statement::Execute while binding parameters");
    }

    if (mysql_stmt_execute(stmt->_stmt) != 0) {
        throw DatabaseException(stmt->_stmt, "Error in Statement::Execute while executing statement");
    }

    if (stmt->_numberResultColumns > 0) {
        if (mysql_stmt_store_result(stmt->_stmt) != 0) {
            throw DatabaseException(stmt->_stmt, "Error in Statement::Execute while storing results");
        }
        stmt->_eof = false;
    }
    else {
        stmt->_numberAffectedRows = mysql_stmt_affected_rows(stmt->_stmt);
    }

    stmt->_resultWasStored = true;

    if (stmt->_hasBlobField && stmt->_resultBind != NULL) {
        MYSQL_RES *res = mysql_stmt_result_metadata(stmt->_stmt);
        if (res != NULL) {
            for (unsigned int i = 0; i < stmt->_resultParams.size(); i++) {
                MYSQL_FIELD *column = mysql_fetch_field_direct(res, i);
                if (stmt->_resultBind[i].buffer_type == MYSQL_TYPE_BLOB) {
                    stmt->_resultParams[i]->ResizeBlob(column->max_length);
                    stmt->_resultBind[i].buffer = stmt->_resultParams[i]->Buffer();
                    stmt->_resultBind[i].buffer_length = stmt->_resultParams[i]->BufferSize();
                    stmt->_resultBind[i].length = stmt->_resultParams[i]->BufferLength();
                }
            }
...
void Statement::ExecuteAsync(SQLCallBack callback)
{
...
void Statement::GetDataInRow(unsigned int column, Nullable<int> &result) {
	...
	if (_resultBind[column].buffer_type != MYSQL_TYPE_LONG) {
        std::cout<<_resultBind[column].buffer_type<<std::endl;
        std::cout<<*((int *) _resultParams[column]->Buffer())<<std::endl;
		throw DatabaseException("Error in Statement::GetDataInRow", 0, "----", "column not of correct type");
	}
